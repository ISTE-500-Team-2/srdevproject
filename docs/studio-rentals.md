...
Only card Checkout is enabled, in test mode. Repeated checkout/refund requests use stable idempotency keys. Refund retries first look for an existing matching provider refund to reconcile a prior lost response. A confirmed failed/canceled provider refund requires staff reconciliation in Stripe; the application will not create another refund against that terminal failure. After completing an original-method refund in the Stripe dashboard, staff can use **Recheck refund after Stripe reconciliation**. The server queries refunds scoped to the original payment intent and records only a provider-confirmed successful USD refund for the full quoted amount, with audit and customer notification. It does not accept a browser-supplied refund reference as proof. A network failure with no confirmed provider refund remains retryable. Unrelated account Checkout events are acknowledged without modifying studios. The callback validates raw-body signatures and rejects live events. Event IDs are persisted atomically with the booking change. Out-of-order pending refund events cannot downgrade a completed refund.
