...
export const rememberedLifetimeSeconds = 7 * 24 * 60 * 60;
...
export class JwtTokens {
  ...
  async issue(userId: number, remember: boolean) {
    if (!Number.isSafeInteger(userId) || userId <= 0)
      throw new Error('Invalid JWT subject');
    const now = Math.floor(Date.now() / 1000);
    const lifetime = remember ? rememberedLifetimeSeconds : sessionLifetimeSeconds;
    const expiresAt = now + lifetime;
    // Identity only. Account status and authorization always come from the DB.
  ...
  async verify(token: string): Promise<{ userId: number } | null> {
    ...
    try {
      const { payload } = await jwtVerify(token, this.key, {
        ...
        maxTokenAge: rememberedLifetimeSeconds,
      });
      if (
        ...
        payload.exp! - payload.iat! > rememberedLifetimeSeconds
      ) return null;
