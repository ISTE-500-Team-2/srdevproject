...
export function AdminAnalyticsPreviewPage() {
  ...
  return (
    <div className="admin-dashboard page-enter">
      <p className="form-notice">
        Design preview: all charts, attendance, and revenue below are sample
        data. These analytics are not connected to live reporting. Use the
        Members, Plans, and Payments tabs for operational records.
      </p>
        ...
        <div className="admin-toolbar">
          ...
          {editing ? (
            <>
              ...
              <button className="button button--primary" onClick={saveLayout}>
                <Save aria-hidden="true" /> Preview layout
              </button>
            </>
          ) : (
            <button
            ...
            >
              <Edit3 aria-hidden="true" /> Edit dashboard
            </button>
          )}
        </div>
      </section>
      ...
      <div className={"admin-layout" + (editing ? " is-editing" : "")}>
        <aside className="admin-sidebar panel">
          <div className="active-users">
            ...
            <i aria-label="Systems operational">
          ...
          <div className="admin-class-list">
            {classes.map((item) => (
              <article className="admin-class" key={item.id}>
                <button
                  onClick={() =>
                    setExpandedClass(expandedClass === item.id ? null : item.id)
                  }
                  aria-expanded={expandedClass === item.id}
                >
                  <span>
                  ...
                  <span className="admin-class__meta">
                    <b>
                      {item.enrolled} / {item.capacity} slots
                    </b>
                    <time>
                      {item.date}
          ...
          <div className="admin-class-list__footer">
            ...
            <strong>
              All Classes <Filter aria-hidden="true" />
            </strong>
          </div>
        </aside>
        ...
        <section className="admin-metrics">
          ...
          <article className="metric-card panel">
            ...
            <div className="metric-card__footer">
              <div>
                <small>Machine Revenue · All Machines</small>
                <strong>$1,874.02</strong>
              </div>
              <select aria-label="Machine revenue period">
                <option>Weekly</option>
                <option>Monthly</option>
              </select>
            </div>
          ...
          <article className="metric-card panel">
            ...
            <div className="metric-card__footer">
              <div>
                <small>Machine Usage · All Machines</small>
                <strong>52.8 hours</strong>
              </div>
              <select aria-label="Machine usage period">
                <option>Weekly</option>
                <option>Monthly</option>
              </select>
            </div>
